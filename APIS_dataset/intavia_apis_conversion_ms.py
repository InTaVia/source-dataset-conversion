...
import datetime as datetime_python
...
base_url_apis = "http://127.0.0.1:8000"
next_page = f"{base_url_apis}/apis/api/entities/person/?limit=50&offset=0"
...
#print(next_page)
#while next_page != f"{base_url_apis}/apis/api/entities/person/?limit=50&offset=100":
...
for index, row in relations_df.iterrows():
    if (row['relationtypeurl']) == f"{base_url_apis}/apis/api/vocabularies/personplacerelation/595/":
...
for index, row in places_df.iterrows():
    g.add((URIRef(f"{idmapis}place/{row['place_id']}"), crm.P168_place_is_defined_by, Literal(f"{row['place_lat']} {row['place_lng']}")))
...
d_file = datetime_python.datetime.now().strftime("%d_%m_%Y")

exapis = g.serialize(destination=f'apisdata_{d_file}.ttl', format='turtle')   
